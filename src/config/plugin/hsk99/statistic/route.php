...
Route::fallback(function ($request) use (&$oldFallback) {
    ...
    $project   = config('plugin.gitfei1231.statistic.app.project');
    ...
    switch (true) {
        case method_exists($response, 'exception') && $exception = $response->exception():
            ...
            break;
        case !empty($response->getHeader('Content-Type')) && strpos(strtolower($response->getHeader('Content-Type')), 'application/json') !== false:
