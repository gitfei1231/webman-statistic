...
class Bootstrap implements \Webman\Bootstrap
{
    ...
    public static function start($worker)
    {
        if ($worker) {
            ...
            self::$_instance = new Client(config('plugin.gitfei1231.statistic.app.http_options', $options));

            // 定时上报数据
            Timer::add(config('plugin.gitfei1231.statistic.app.interval', 30), function () {
                Statistic::report();
            });

            // 执行监听所有进程 SQL、Redis
            if (config('plugin.gitfei1231.statistic.app.global_monitor', false)) {
